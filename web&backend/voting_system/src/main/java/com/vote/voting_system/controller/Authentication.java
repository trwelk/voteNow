...
public class Authentication {
	...
	public ResponseEntity<?> loginMobile(@RequestBody String str){
		...
		Optional<Politicion> allData=null;
		...
		try {
			...
			if(getAll.isPresent()) {
				if(getAll.get().getStatus() == 1 && getAll.get().getRoleBean().getId() == 2) {
					...
					Object.put("role", getAll.get().getRoleBean().getId());
					Object.put("person_id", getAll.get().getPerson().getId());
					
					citizenData=citizenService_01.findById(getAll.get().getPerson());
					allData=politicionService_01.findById(getAll.get().getPerson().getId());
					...
					if(citizenData.isPresent()) {
						...
						Object.put("c_status", citizenData.get().getStatus());
						Object.put("p_type", "1");
					}
					else if(allData.isPresent()) {
						Object.put("poli_id", allData.get().getId());
						Object.put("reffno", allData.get().getReffCode());
						Object.put("otp_code", allData.get().getOtpCode());
						Object.put("team_id", allData.get().getTeam().getId());
						Object.put("team_name", allData.get().getTeam().getName());
						Object.put("p_status", getAll.get().getStatus());
						Object.put("p_type", "2");
					}else {
						Object.put("p_type", "3");
					}
...
//		array.put(obj);
	...
	@PostMapping(value = "/vote_time_check")
	public String voteTimeCheck(@RequestParam ("c_id") int citizen_id) {
		Optional<Citizen> getAll;
		JSONArray array=new JSONArray();
//		System.out.println(citizen_id);
//		System.out.println(code);
		...
		try {
			...
			getAll=citizenService_01.findBystatus(citizen_id);
			if(getAll.isPresent()) {
					if(getAll.get().getStatus() == 1) {
//						JSONObject obj=new JSONObject();
						obj.put("status", getAll.get().getStatus());
						obj.put("message", "Active Time now..");
					}else if(getAll.get().getStatus() == 2) {
						obj.put("status", getAll.get().getStatus());
						obj.put("message", "Deactiveted Time now..");
					}

			}else {
				
				obj.put("result", "0");
				obj.put("message", "no such A Citizen");
//				System.out.println("Null");
			}
		} catch (Exception e) {
			System.out.println(e);

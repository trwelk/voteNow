...
//				politicionsHasElectionRepository.delete(entity);
		...
		@ResponseBody
		@GetMapping(value = "/get_polition_list_vote")
		public String getAllPolitionList_vote_results() {
			JSONArray array=new JSONArray();
			List<PoliticionsHasElection> getAll;
			List<Vote> getVoteAll = null;
			try {
				getAll=politicionsHasElectionService_01.findByAll();
				if(!getAll.isEmpty()) {
					
					for(PoliticionsHasElection c: getAll) {

						getVoteAll=c.getVotes();
						JSONObject obj=new JSONObject();
						if(c.getPoliticion().getPerson().getStatus() == 1) {
							JSONObject object=new JSONObject();
							
							int vote_count=voteService_01.countByPoliticionHas(c.getId());
							object.put("id", c.getId());
							object.put("poli_status", c.getStatus());
							object.put("vote_count", vote_count);
							object.put("person_fname", c.getPoliticion().getPerson().getFname());
							object.put("person_lname", c.getPoliticion().getPerson().getLname());
							object.put("person_id", c.getPoliticion().getPerson().getId());
							object.put("team_id", c.getPoliticion().getTeam().getId());
							object.put("team_name", c.getPoliticion().getTeam().getName());
							object.put("election_number", c.getNumberEle());
							object.put("district_name", c.getDistrict().getName());
							array.put(object);
						}
					}
...
//					getVoteAll=voteService_01.findByPoliticionsHasElection(getAll);
//						if(!getVoteAll.isEmpty()) {
//							for(Vote v : getVoteAll) {
//								voteService_01.countByPoliticionHas(v.getPoliticionsHasElection().getId());
//							}
//							
						}
